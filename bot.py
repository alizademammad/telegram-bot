...
secretary_active = True  # پیش‌فرض فعال!
...
async def handle_secretary(event):
    sender = event.sender_id
    print(f"📥 Secretary check - sender: {sender}, is_admin: {sender == ADMIN_ID}, secretary_active: {secretary_active}, is_group: {event.is_group}, is_channel: {event.is_channel}")

    if sender == ADMIN_ID:
        print("⏭️ Skipped: admin message")
        return
    if not secretary_active:
        print("⏭️ Skipped: secretary not active")
        return
    if event.is_group or event.is_channel:
        print("⏭️ Skipped: group/channel message")
        return

    chat_id = sender
    print(f"🤖 Secretary: scheduling reply to {chat_id}")
