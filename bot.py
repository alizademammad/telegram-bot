...
async def handle_all(event):
    ...
    # ─── لاگ ───
    print(f"📨 From:{sender} | private:{is_private} | msg:{msg[:50]}")

    # ─── کلیک دکمه اینلاین (ادمین) ───
    if is_admin and event.is_private:
        if msg == "پنل":
            await client.send_message(event.chat_id, get_panel_text(), buttons=get_panel_markup())
            return

    # ─── اگه ادمین نیست و پیام خصوصیه → منشی ───
    if not is_admin and is_private:
        if secretary_active:
            print(f"🤖 Secretary: scheduling reply to {sender}")
            if sender in pending_replies:
                pending_replies[sender].cancel()
            task = asyncio.create_task(secretary_reply(sender))
            pending_replies[sender] = task
        else:
            print(f"⏭️ Secretary off, skipping {sender}")
        return
...
async def handle_callback(event):
    ...
    if data == "toggle_online":
        ...
        await event.answer()
        await client.send_message(event.chat_id, f"📝 متن‌های منشی (تصادفی):\n\n{texts}")
...
async def main():
    ...
    if not SESSION_STRING:
        print("\n" + "="*50)
        print("⚠️  SESSION STRING رو کپی کن!")
        print("="*50)
